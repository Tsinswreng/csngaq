...
namespace ngaq.Infra.dddSample.email;

public class MimeKitEmailSender(ILogger<MimeKitEmailSender> logger,
  IOptions<MailserverConfig> mailserverOptions) : I_sendEmailAsy
{
  private readonly ILogger<MimeKitEmailSender> _logger = logger;
  private readonly MailserverConfig _mailserverConfiguration = mailserverOptions.Value!;

  public async Task sendEmailAsy(string to, string from, string subject, string body)
  {
